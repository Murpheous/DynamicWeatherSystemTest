...
public class ParticleArrayMesh : MonoBehaviour
{
    ...
    private bool generateMesh()
    {
        ...
        if (material != null)
        {
            if (material.HasProperty("_ArraySpacing"))
                material.SetVector("_ArraySpacing", arraySpacing);
            if (material.HasProperty("_ArrayDimension"))
            {
                // Tell shader pointcount in each direction if required;
                Vector4 pointVec = new Vector4(numGridPoints.x, numGridPoints.y, numGridPoints.z, useTriangles ? 3f : 4f);
